...
const coreHelp: HelpProvider = () => [
  { id: 'start', title: 'Find your way around', category: 'Getting started', summary: 'Files, Studio and your everyday screens.', related: ['overview', 'records', 'files'], sections: [
    ...
    { heading: 'Custom views', paragraphs: [
      ...
      'A view reads the file through the same typed services Studio uses, calculated fields and exact numbers included. It can open a record, a screen or Studio and show a short message. It can also change records and run a record’s commands, the way you would: each change is checked against the record’s version and shows in History under the view’s package, where you can undo it. Nendo asks nothing first; a view that wants you to confirm asks you itself. It is a web page in a frame of its own, so it can also reach the network and the clipboard. A view that stops answering says so, with Stop and Reload, and the rest of Nendo keeps working.',
      'A view can also ask for a change to the app itself, such as a field it needs. Nendo opens the ordinary review at once and says which view asked; nothing changes until you accept, and a view can never accept or reject on your behalf. Each view waits for your answer before it asks again, and History shows the accepted change under the view’s package.',
      'A view can keep small values with the file, such as a layout or a choice, so a copy of the file carries them. Each change shows in History under the view’s package, and you can undo it there.',
      ...
      'Each package in that panel lists the screens and record pages that show it. Add view… on its card puts it somewhere new: choose a screen of records, a graph or a panel on a record page, then the record type and the fields it shows. Only fields that work are offered. Preview view opens the review, and once you accept a screen, Use opens on it.',
      'To write a view, import its package once, then press Develop from folder… on its card and pick the package’s folder. On this computer every view of the package then runs from that folder, under a Development strip, and reloads whenever you save a file there. The file is not changed: Save to file… prepares a proposal you review, and Stop developing puts the file’s code back.',
    ] },
  ] },
