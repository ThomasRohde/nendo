...
const coreHelp: HelpProvider = () => [
  { id: 'start', title: 'Find your way around', category: 'Getting started', summary: 'Files, Studio and your everyday screens.', related: ['overview', 'records', 'files'], sections: [
    ...
    { heading: 'Offline custom views', paragraphs: ['File → Custom views installs, exports or removes a package on this device. Installation does not let it run: review permission for the configured view and check the fields it will read. Its screen in Use shows one next step at a time: install the package from its file, allow the view, then open the graph beside your records. Manage packages covers export, removal and disabling. The file needs the exact package version and digest it names.', 'In a graph, select a node and choose Open record to edit it in Nendo. F6 moves focus between the graph and Nendo’s controls. Studio remains available if a package is missing, disabled or stops working. Disable view withdraws permission on this device; your records stay in the file. Export the package separately when moving to another device, where permission must be reviewed again.'] },
  ] },
