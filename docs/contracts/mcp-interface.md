...
| `nendo.data.import_records` | Same | `ImportAsync` → `NendoImportService` → `CreateRecordsAsync`. Takes CSV text or typed JSON. The routine that the native importer uses decodes the input, and the same canonical `data.createRecord` operations commit it, fifty to a revision. At most 500 rows per call; the 256 KiB body is the limit that is met first. `maximumRowsPerCall` echoes the limit. The idempotency key of each batch is derived from the caller's key. The record ID of a CSV row is derived from that key and the position of the row, so an exact retry asks for the same records and not for a second copy. A refused batch stops the run. `committed` says how many records were committed, and the response never implies that the call was atomic. |
