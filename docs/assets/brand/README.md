...
The Workbench empty states animate the static `nendo-mark.png` in CSS instead (`brand-mark-settle` and `brand-mark-breathe` in `src/Nendo.Workbench/src/styles.css`). The mark drops in, squashes and settles like moulded clay. Then it continues with a slow idle breath. The animation uses only transforms and inherits the effective theme. It adds no payload. Under `prefers-reduced-motion`, it becomes a still mark. The static mark remains the default documentation and app identity asset.
