...
$infoPath = Join-Path $output "systems-lens-server-$run.json"
$probePath = Join-Path $output "systems-lens-probe-$run.mjs"
$serverScript = Join-Path $PSScriptRoot 'Graph-FixtureServer.mjs'
$server = Start-Process node -ArgumentList @("`"$serverScript`"", "`"$infoPath`"", '../extensions/systems-lens', 'index.html,lens.js,lens.css') -WindowStyle Hidden -PassThru
$session = "systems-lens-$run"
Push-Location $output
try {
    $deadline = [DateTime]::UtcNow.AddSeconds(10)
    while (-not (Test-Path -LiteralPath $infoPath)) {
        if ($server.HasExited -or [DateTime]::UtcNow -gt $deadline) { throw 'Systems Lens fixture server did not start.' }
        Start-Sleep -Milliseconds 100
    }
    $info = Get-Content -LiteralPath $infoPath -Raw | ConvertFrom-Json
    if ($info.pid -ne $server.Id) { throw 'Systems Lens fixture process identity differs.' }
    $probe = [IO.File]::ReadAllText((Join-Path $PSScriptRoot 'Gate-SystemsLens.mjs'))
    ...
    if ($LASTEXITCODE -ne 0) { throw 'Systems Lens browser did not start.' }
    & npx.cmd --yes --package '@playwright/cli@0.1.21' playwright-cli "-s=$session" run-code --filename $probePath
    if ($LASTEXITCODE -ne 0) { throw 'Systems Lens browser measurements failed.' }
} finally {
