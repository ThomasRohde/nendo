...
$server = Start-Process node -ArgumentList @("`"$serverScript`"", "`"$infoPath`"") -WindowStyle Hidden -PassThru
$session = "graph-$run"
...
try {
    ...
    $probe = $probe.Replace("'__NENDO_REPOSITORY__'", ($repoRoot.Replace('\', '/') | ConvertTo-Json -Compress)).Replace('__GRAPH_BASE_URL__', $info.url)
