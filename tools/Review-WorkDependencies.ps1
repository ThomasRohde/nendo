...
try {
    ...
    while (-not (Test-Path -LiteralPath $infoPath)) {
        if ($server.HasExited -or [DateTime]::UtcNow -gt $deadline) { throw 'Work-dependency fixture server did not start.' }
    ...
    if ($LASTEXITCODE -ne 0) { throw 'Work-dependency browser measurements failed.' }
} finally {
