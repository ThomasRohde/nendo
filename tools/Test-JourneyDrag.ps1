...
$evidenceRoot = Join-Path $repoRoot ('artifacts/evidence/runs/p5-drag-' + [Guid]::NewGuid().ToString('N'))
...
foreach ($phase in @('create', 'reopen')) {
    ...
    try {
        ...
        & node (Join-Path $PSScriptRoot 'Journey-Drag.mjs') $port $target.Id $evidenceRoot $phase
